...
import coffee.khyonieheart.hyacinth.module.ClassCollector;
 ...
 * @since 1.0.0
 */
public class HyacinthModuleManager<T extends Command & ModuleOwned> implements ModuleManager, Chainloadable
{
    ...
    public HyacinthModule loadModule(@NotNull File moduleFile) throws IllegalArgumentException, FileNotFoundException, IOException, HyacinthModuleException
    {
        ...
        if (moduleConfig.contains("requires"))
        {
            ...
            for (String dependency : requires)
            {
                ...
                try {
                    ...
                    Logger.verbose("§cDependency " + dependency + " of module " + moduleConfig.getString("name") + " failed to load with exception: " + e.getClass().getSimpleName());
                    missingDependencies.add(dependency);
        ...
        Map<Class<?>, List<Class<?>>> toRegister = hmcl.collectMultiSubclasses(Command.class, Listener.class, Feature.class, ClassCollector.class);
		...
		// Then commands and listeners

        int commandCount = toRegister.get(Command.class).size();
        ...
        // Commands
        for (Class<?> commandClass : toRegister.get(Command.class))
        {
			Logger.verbose(" - Registering command " + commandClass.getName());
            T command;

            try {
                command = (T) Reflect.simpleInstantiate(commandClass);
		...
		// Killswitch targets
		for (Class<?> featureClass : toRegister.get(Feature.class))
		{
			Logger.verbose(" - Registering feature " + featureClass.getName());
			Feature target;

			try {
				target = (Feature) Reflect.simpleInstantiate(featureClass);
			} catch (InstantiationRuntimeException e) {
				encounteredErrors = true;
			...
			FeatureManager.register((Class<? extends HyacinthModule>) moduleClass, target);
		}
	...
	public YamlConfiguration getConfiguration(
		...
		for (HyacinthModule mod : this.loadedModules.values())
		{
			if (mod.getClass().equals(module))
			{
				return getConfiguration(mod);
			}
		}
