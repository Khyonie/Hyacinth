...
public class OrigamiModuleClassloader extends URLClassLoader
{
    private Map<String, Class<?>> interiorClasses = new HashMap<>();

    private JarFile jar;

    public OrigamiModuleClassloader(File hostFile, JarFile jar, ClassLoader parent) throws MalformedURLException 
    {
    ...
    public Class<?> findClass(String name, boolean checkGlobal) throws ClassNotFoundException
    {
        ...
        if (checkGlobal)
        {
            result = Origami.getModuleManager().getGlobalClass(name, this);
    ...
    public static OrigamiModuleClassloader create(File file, JarFile jar) throws MalformedURLException
    {
        if (file == null)
